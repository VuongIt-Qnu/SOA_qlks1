@echo off
REM Script để fix lỗi InnoDB corruption trong MySQL containers
REM Sử dụng script này khi MySQL container không thể khởi động do lỗi InnoDB
...
set /p choice="Bạn có muốn tiếp tục? (y/n): "
if /i not "%choice%"=="y" (
    echo Đã hủy.
    exit /b
)
...
docker volume ls | findstr "_db_data"

echo.
set /p volume_name="Nhập tên volume cần xóa (ví dụ: soa_qlks_booking_db_data) hoặc 'all' để xóa tất cả: "

if /i "%volume_name%"=="all" (
    echo Đang xóa tất cả MySQL volumes...
    docker volume rm soa_qlks_auth_db_data 2>nul
    docker volume rm soa_qlks_customer_db_data 2>nul
    docker volume rm soa_qlks_room_db_data 2>nul
    docker volume rm soa_qlks_booking_db_data 2>nul
    docker volume rm soa_qlks_payment_db_data 2>nul
    docker volume rm soa_qlks_report_db_data 2>nul
    ...
    if not "%volume_name%"=="" (
        ...
        if %errorlevel%==0 (
            echo Đã xóa volume thành công.
        ) else (
            echo Không thể xóa volume. Có thể volume không tồn tại hoặc đang được sử dụng.
        )
    )
)
